...
def write_wav(y, sr, save_path):
    ...
    wav.write(filename=save_path, rate=sr, data=np.array(
        np.clip(np.round(y), -2**15, 2**15-1), dtype=np.int16))
...
#     print(y[:10])
    # 返回DFT后每个值对应的频率； 即 k*sr/n = k/d*n，k表示时间序列的索引值；(0, sr/n, 2*sr/n, ..., sr/2)
    f = np.fft.fftfreq(n=n, d=1/sr)[:n//2+1]
    f[-1] = f[-1] * -1
...
def stft(audio):
    ...
    audio_pad = librosa.util.fix_length(
        audio, n + n_fft // 2)  # 为了方便STFT，对音频数据进行末尾0填充
...
#     L = N // 10                                             # 选取适当窗口长度，取在[2, N//3]之间，是轨迹矩阵的行数
    ...
    # 返回特征值的降序排序后对应的索引值排序
    index_sort = np.argsort(s)[::-1]
    u_ = u[:, index_sort]
    # 按降序后的索引值为特征值和特征向量分别排序; （注： 特征向量的每一列对应于一个特征值，而非每一行）
    s_ = s[index_sort]
...
def seq_to_frame(y, frame_length=2048, hop_length=None):
    ...
    hop_length； 设置前后两帧开始位置相差的长度；
    返回：分帧后的数组，形状shape(帧数, frame_size)
    """
    n = y.shape[0]                                                              # 音频数据的长度
    if hop_length is None:
        hop_length = int(frame_length // 4)
    pre_frame_num = int(np.ceil((n - frame_length) /
                                hop_length + 1))           # 预计的帧的数量
    pre_y_len = hop_length * (pre_frame_num - 1) + \
        frame_length                 # 预计的音频数据的长度（≥n）
    y_pad = np.pad(y, (0, pre_y_len-n), mode='constant',
...
def frame_to_seq(y_frames, hop_length=None, org_len=None):
    ...
    # 若是给了音频序列原始长度，在还原其后，需要截掉分帧时填充的部分
    if org_len is not None:
        x = x[:org_len]
    return x

# x, sr = read_wav("sample.wav")
# print(x)
# print(x.shape)
# n = x.shape[0]
# frame_length = 2048
