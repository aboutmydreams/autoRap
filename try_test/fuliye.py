...
S = 0.2+0.7*np.cos(2*np.pi*50*t+20/180*np.pi) + 0.2 * \
    np.cos(2*np.pi*100*t+70/180*np.pi)
...
plt.xlabel("t（毫秒）", fontproperties=my_font)
plt.ylabel("S(t)幅值", fontproperties=my_font)
plt.title("叠加信号图", fontproperties=my_font)
plt.legend()
...
plt.xlabel("t（毫秒）", fontproperties=my_font)
plt.ylabel("S_ifft(t)幅值", fontproperties=my_font)
plt.title("ifft变换图", fontproperties=my_font)
plt.grid(linestyle=':')
plt.legend()
...
plt.subplot(313)
plt.title('FFT变换,频谱图', fontproperties=my_font)
plt.xlabel('Frequency 频率', fontproperties=my_font)
plt.ylabel('Power 功率', fontproperties=my_font)
