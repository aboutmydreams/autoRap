...
def enframe(wave_data, nw, inc, winfunc):
    ...
    indices = np.tile(np.arange(0, nw), (nf, 1))+np.tile(np.arange(0,
                                                                   nf*inc, inc), (nw, 1)).T  # 相当于对所有帧的时间点进行抽取，得到nf*nw长度的矩阵
...
# 分帧、加窗
winfunc = signal.windows.hann(256)
...
for i in range(frameNum):
    ...
    mel_points = np.linspace(
        low_freq_mel, high_freq_mel, nfilt + 2)  # 将梅尔刻度等间隔
    ...
    # DCT系数
    num_ceps = 12
    c2 = dct(filter_banks, type=2, axis=-1,
...
plt.plot(np.arange(len(wave_data)), wave_data, 'red')
plt.xlabel("time (s)")
plt.show()
